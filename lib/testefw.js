...
const prepararTestes = (() => {
    ...
    class Utilitarios {
        ...
        static stringify(dados) {
            const prev = [];
            function inner(dados) {
                const idx = prev.indexOf(dados);
                prev.push(dados);
                try {
                    if (idx !== -1) {
                        return "#" + idx;
                    ...
                    if (dados instanceof Array) {
                        return "[" + dados.map(inner).join(", ") + "]";
                    }
                    if (dados !== null && typeof dados === "object") {
                        if (dados.constructor !== Object) dados = Utilitarios.extractGetters(dados, a => a.sort());
                        const props = [];
                        for (const p in dados) {
                            props.push(p);
                        }
                        props.sort();
                        return "{" + props.map(p => `${inner(p)}: ${inner(dados[p])}`).join(", ") + "}";
                    }
                    return "" + dados;
                } finally {
                    prev.pop();
    ...
    class Grupo {
        ...
        constructor(nome, subtitulo, fracionado, pararNoPrimeiroErro, minima, maxima, testes, atualizarSuite) {
            ...
            const elementoHTML = ``
                    ...
                    + `<h3 class="testefw-cabecalho">${nome}: <span class="testefw-sucessos">0</span> / ${this.#testes.length}</h3>`
                    + `<h4 class="testefw-cabecalho">${subtitulo}</h4>`
                    + `<h4 class="testefw-cabecalho ${this.#minima === 0 ? "" : "testefw-perigo"}">${subPesoTexto}</h4>`
                    + `<ul></ul>`
                    + `</li>`;
    ...
    class Teste {
        ...
        constructor(regra, execucao, testeDoResultado, preCondicao, posOperacao) {
            ...
            const elementoHTML = ""
                    ...
                    + `<div class="testefw-codigo">Código:<pre>${escapeCodigo}</pre></div>`
                    + `<div class="resultado-esperado">${testeDoResultado.esperado(false)}</div>`
        ...
        async executar(scroll) {
            ...
            try {
                ...
                const esperado = this.#testeDoResultado.esperado(true);
                ok = true;
                return this.#atualizarHtml("ok", esperado, `Resultado obtido: <span class="testefw-funcionou">Funcionou como esperado.</span>`, scroll);
            } catch (e) {
    ...
    function naoDeuErro() {
        return {
            testar: resultado => {},
            esperado: jaExecutou => `Resultado esperado: Apenas executar corretamente e não lançar nenhum erro.`
        };
    }
    ...
    const painelHTML = ""
            ...
            + '        <div>'
            + '            <h2>Nota: <span id="testefw-valor">0</span> / <span id="testefw-valormax">0</span></h2>'
            ...
            + '        </div>'
            + '        <div>'
            + '            <h3>Sucessos: <span id="testefw-contasucessos">0 / 0</span></h3>'
            + '            <h3>Erros: <span id="testefw-contaerros">0 / 0</span></h3>'
            + '            <h3>Pulados: <span id="testefw-contapulos">0 / 0</span></h3>'
            + '            <h3>Não implementados: <span id="testefw-contaniy">0 / 0</span></h3>'
            + '            <h3>Aguardando: <span id="testefw-contaespera">0 / 0</span></h3>'
            + '        </div>'
            + '    </div>'
    ...
    class Suite {
        ...
        #criarFormExecutarTestes() {
            const antigo = document.querySelector("#testefw-delay");
            const delayAntigo = antigo ? antigo.value : "-1";
            const conteudoForm = ""
                    + '<p>'
            ...
            document.querySelector("#testefw-form-executar").innerHTML = conteudoForm;
            ...
            return {
                testar: valorObtido => {
                    if (!valorObtido.includes(sucesso)) {
                        throw new ErroFormatado(sucesso, valorObtido);
                    }
                },
                esperado: jaExecutou => `Resultado esperado: ${Utilitarios.escapeHtml(sucesso)}.`
            };
        }
    }
    ...
    class ErroFormatado extends Error {
        ...
        constructor(valorEsperado, valorObtido) {
            ...
            const v1 = Utilitarios.stringify(valorEsperado);
            const v2 = Utilitarios.stringify(valorObtido);
            const e1 = Utilitarios.escapeHtml(v1);
            const e2 = Utilitarios.escapeHtml(v2);
            let esperado, obtido;
            if (t1 === t2 && v1 === v2) {
                esperado = `Resultado esperado: [${t1}] - ${e1}.`;
                obtido = `Resultado obtido: [${t2}] - ${e2}.<br>`
                        + `<span class="testefw-destaque">`
                        + `Entretanto, ainda assim a comparação de igualdade entre o obtido e o esperado resultou em false. Isso nunca deveria acontecer!`
                        + `</span>`
            } else if (t1 === t2) {
                esperado = `Resultado esperado: <span class="testefw-destaque">${e1}</span>.`;
                obtido = `Resultado obtido: <span class="testefw-destaque">${e2}</span>.`;
            } else if (v1 === v2) {
                esperado = `Resultado esperado: <span class="testefw-destaque">[${t1}]</span> - ${e1}.`;
                obtido = `Resultado obtido: <span class="testefw-destaque">[${t2}]</span> - ${e2}.`;
            } else {
                esperado = `Resultado esperado: <span class="testefw-destaque">[${t1}]</span> - <span class="testefw-destaque">${e1}</span>.`;
                obtido = `Resultado obtido: <span class="testefw-destaque">[${t2}]</span> - <span class="testefw-destaque">${e2}</span>.`;
            }
